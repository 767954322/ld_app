 ...
 * @author clifford
 */
public class MPFileHotspotView extends ImageView {
    ...
    ////////////////////////////////Data from SignImageHotSpotsActivity////////////////////////////////
    ...
    private float hot_icon_width;
    private float img_magnification;

    private int org_img_width;
    private int org_img_height;

    private RectF currentImageInfo;
    private RectF originalImageInfo;

    private Point currentPt = new Point();

    private HotSpotImageViewListender mHotSpotImageViewListender;
    ...
    //this is required when we want to get orginal image bounds
    // otherwise point calculation does not work on image

    public RectF getOrgImageBound(RectF rectF) {
        if (rectF == null) {
            rectF = new RectF();
        } else {
            rectF.setEmpty();
    ...
    /**
     * 设置signImageHotSpotsActivity部分控件
     */
    public void setDataFromImageSingActivity(Activity activity, MPFileHotspotActivity.HotspotImageData imageData) {

        this.activity = activity;
        this.currentImageInfo = new RectF();
        this.originalImageInfo = new RectF();
        ...
        isNormalType = false;
        getOrgImageBound(originalImageInfo);
    }
    ...
    public interface OuterMatrixChangedListener {
         ...
         */
        void onOuterMatrixChanged(MPFileHotspotView pinchImageView);
    }
    ...
    public MPFileHotspotView(Context context) {
        super(context);
        initView();
    }

    public MPFileHotspotView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initView();
    }

    public MPFileHotspotView(Context context, AttributeSet attrs, int defStyle) {
    ...
    private boolean isDoingAnimation() {
        ...
        return (mScaleAnimator != null && mScaleAnimator.isRunning()) || (mFlingAnimator != null && mFlingAnimator.isRunning());
     ...
     * 判断点击的座标是否在可显示的范围内
     */
    private boolean isLegalPosition(Point pt) {

        this.getImageBound(currentImageInfo);

        if (currentImageInfo.width() >= this.getWidth() && currentImageInfo.height() >= this.getHeight()) {

            return true;

        } else {

            if (currentImageInfo.width() < this.getWidth()) {

                return !(pt.x < currentImageInfo.left || pt.x > currentImageInfo.right);

            } else if (currentImageInfo.height() < this.getHeight()) {

                return !(pt.y < currentImageInfo.top || pt.y > currentImageInfo.bottom);
     ...
     * 在onTouchEvent末尾被执行.
     */
    private GestureDetector mGestureDetector = new GestureDetector(MPFileHotspotView.this.getContext(), new GestureDetector.SimpleOnGestureListener() {
        ...
        public void onLongPress(MotionEvent e) {
            ...
            if (mOnLongClickListener != null) {
                mOnLongClickListener.onLongClick(MPFileHotspotView.this);
            }
        }
        ...
        public boolean onSingleTapConfirmed(MotionEvent e) {
            ...
            if (mOnClickListener != null) {
                mOnClickListener.onClick(MPFileHotspotView.this);
            }
            return true;
        }
    });
     ...
     * 设置热点图片是否启用触控监听事件
     *
     * @param isEnableTouch isEnableTouch
     */
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (action == MotionEvent.ACTION_UP || action == MotionEvent.ACTION_CANCEL) {
            ...
            if (!isMoving && !isDoingAnimation() && !isNormalType) {

                Point tempPt = new Point((int) event.getX(), (int) event.getY());

                if (isLegalPosition(tempPt)) {

                    //转换为相对原尺寸座标
                    getImageBound(currentImageInfo);
                    currentPt.x = (int) (((tempPt.x - currentImageInfo.left) / currentImageInfo.width()) * org_img_width);
                    currentPt.y = (int) (((tempPt.y - currentImageInfo.top) / currentImageInfo.height()) * org_img_height);

                    if (mHotSpotImageViewListender != null && mHotSpotImageViewListender.shouldInsertNewLocation()) {

                        hotSpots_frame.setVisibility(View.VISIBLE);

                        RelativeLayout.LayoutParams rl = (RelativeLayout.LayoutParams) hotSpots_frame.getLayoutParams();
                        rl.leftMargin = (int) (tempPt.x - hot_icon_width / 2);
                        rl.topMargin = (int) (tempPt.y - hot_icon_width);
                        hotSpots_frame.setLayoutParams(rl);
                        hotSpots_frame.invalidate();

                        mHotSpotImageViewListender.didFinishwithAddingNewHotSpot(currentPt);
                    }
                }
            }
            ...
            if (mPinchMode == PINCH_MODE_SCALE) {
                ...
                if (event.getPointerCount() > 2) {
                    ...
                    if (outerScale < 1) {
                        ...
                        if (currentImageInfo == null || currentImageInfo.left > 0 || currentImageInfo.top > 0 || currentImageInfo.right < this.getWidth() || currentImageInfo.bottom < this.getHeight()) {
                        ...
                        if (event.getAction() >> 8 == 0) {
                            ...
                            mLastMovePoint.set(event.getX(1), event.getY(1));
            ...
            if (!(mScaleAnimator != null && mScaleAnimator.isRunning())) {
                ...
                if (mPinchMode == PINCH_MODE_SCROLL) {
                    ...
                    if (MathUtils.getDistance(event.getX(0), event.getY(0), mLastMovePoint.x, mLastMovePoint.y) > 10) {
                    ...
                    if (MathUtils.getDistance(lineCenter[0], lineCenter[1], mLastMovePoint.x, mLastMovePoint.y) > 10) {
     ...
     * 取得热点缩放后的边长
     *
     * @param currentScale 当前图像缩放寸尺
     */
    private int getHotSpotWH(float currentScale) {

        float min_width = hot_icon_width * 0.95f;
        float max_width = hot_icon_width * 1.05f;

        int result;

        if (currentScale > 1) {

            result = (int) (((min_width - hot_icon_width) / (MAX_SCALE - 1)) * (currentScale - 1) + hot_icon_width);

        } else {

            result = (int) (((max_width - hot_icon_width) / (MIN_SCALE - 1)) * (currentScale - 1) + hot_icon_width);
     ...
     * 新增HotSpot
     *
     * @param thread_id 每个热点聊天室的id
     * @param view      HotSpot
     */
    public void addHotSpots2Map(final String thread_id, final View view, final Point pt ) {

        updateHotSpotPosition(view,pt);
    }
     ...
     * @param view HotSpot
     */
    private void updateHotSpotPosition(View view, Point pt) {
        ...
        tempPt.x = (int) (((float) pt.x / (float) org_img_width) * originalImageInfo.width());
        ...
        view.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (mHotSpotImageViewListender != null) {
                    mHotSpotImageViewListender.tapOnHotSpot(v);
                }
            }
        });


        updatePointAnimation(view);
     ...
     * 更新热点父控件长宽与位置
     */
    private void updateHotSpotsParentPosition() {
...
//            singHotSpotImageView_mask.addView(view);
     ...
     * @param view HotSpot
     */
    private void updatePointAnimation(View view) {

        AnimatorSet animatorSet = new AnimatorSet();//组合动画
        ObjectAnimator ax = ObjectAnimator.ofFloat(view, "scaleX", 0.5f, 1.2f);
        ObjectAnimator _ax = ObjectAnimator.ofFloat(view, "scaleX", 1.2f, 1f);

        ObjectAnimator ay = ObjectAnimator.ofFloat(view, "scaleY", 0.5f, 1.2f);
        ObjectAnimator _ay = ObjectAnimator.ofFloat(view, "scaleY", 1.2f, 1f);

        animatorSet.setDuration(500);

        animatorSet.play(ax).before(_ax);
        animatorSet.play(ay).before(_ay);
    ...
    public void setHotSpotImageViewListener(HotSpotImageViewListender listener) {
        ...
        mHotSpotImageViewListender = listener;
    }


    public interface HotSpotImageViewListender {
        boolean shouldInsertNewLocation();
        void didFinishwithAddingNewHotSpot(Point pt);
        void tapOnHotSpot(View view);
    }
