 ...
 * @author Malidong .
 * @version 1.0 .
 * @date 2016/1/21 0021 13:39 .
 * @file DesignerPersonalCenterFragment  .
 * @brief 设计师个人中心fragment.
 */
public class DesignerPersonalCenterFragment extends BaseFragment implements View.OnClickListener {
    ...
    protected void initView() {
        ...
        mLlNoAttestation = (LinearLayout) rootView.findViewById(R.id.ll_no_attestation);
        mPolygonImageView = (PolygonImageView) rootView.findViewById(R.id.ib_designer_photo);
        mTvAuditStatusAgo = (TextView) rootView.findViewById(R.id.tv_audit_status_ago);
        mImgCertificateIcon = (ImageView) rootView.findViewById(R.id.img_autonym_icon);
        mTvDesignerNickname = (TextView) rootView.findViewById(R.id.tv_designer_nickname);
    }
        ...
        switch (v.getId()) {
            case R.id.ib_designer_photo:    /// 进入设计师个人信息页面.
                if (null != memberEntity && Constant.UerInfoKey.DESIGNER_TYPE.equals(memberEntity.getMember_type()) && designerInfoDetails != null) {
                    Intent intent = new Intent(getActivity(), DesignerEssentialInfoActivity.class);
...
//                Intent aaIntent = new Intent(getActivity(),CertificationActivity.class);
...
//                startActivity(aaIntent);
                if (audit_status == null) {
                    CommonUtils.launchActivity(getActivity(), CertificationActivity.class);
                } else if (audit_status.equals(Constant.NumKey.ZERO)) {
                    Intent intent = new Intent(getActivity(), AttestationInfoActivity.class);
    ...
    private void scanner() {
        if (checkCameraHardware(activity)) {
            Intent intent = new Intent(activity, CaptureQrActivity.class);
            startActivityForResult(intent, QR);/// Jump to the qr code scanning page.
        } else {
            new AlertView(UIUtils.getString(R.string.tip), UIUtils.getString(R.string.camera_is_not_available), null, null, new String[]{UIUtils.getString(R.string.sure)}, getActivity(), AlertView.Style.Alert, null).show();
        }
    }
                ...
                case QR:
                    ...
                    if (scanResult.contains(Constant.ConsumerDecorationFragment.hs_uid) && scanResult.contains(Constant.DesignerCenterBundleKey.MEMBER)) {
                        ...
                        if (null != consumerQrEntity && !TextUtils.isEmpty(consumerQrEntity.getName())) {
                            ...
                            Intent intent = new Intent(getActivity(), BeiShuMealActivity.class);
                            startActivity(intent);
                        }
                    } else {
                        new AlertView(UIUtils.getString(R.string.tip), UIUtils.getString(R.string.unable_create_beishu_meal), null, null, new String[]{UIUtils.getString(R.string.sure)}, getActivity(), AlertView.Style.Alert, null).show();
                    }
                    break;
