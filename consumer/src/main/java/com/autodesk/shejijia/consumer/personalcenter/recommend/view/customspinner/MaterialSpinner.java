package com.autodesk.shejijia.consumer.personalcenter.recommend.view.customspinner;
...
public class MaterialSpinner extends TextView {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        boolean rtl = SpinnerUtils.isRtl(context);
        ...
        try {
            ...
            arrowColorDisabled = SpinnerUtils.lighter(arrowColor, 0.8f);
        } finally {
            ta.recycle();
        }
        ...
        left = right = bottom = top = resources.getDimensionPixelSize(R.dimen.size_5);
        if (rtl) {
            right = resources.getDimensionPixelSize(R.dimen.size_5);
        } else {
            left = resources.getDimensionPixelSize(R.dimen.size_5);
        }

        setGravity(Gravity.CENTER_VERTICAL | Gravity.START);
        setClickable(true);
        setPadding(left, top, right, bottom);
        setBackgroundResource(R.drawable.edit_text_selector);
        ...
        if (!hideArrow) {
            arrowDrawable = SpinnerUtils.getDrawable(context, R.drawable.ms_arrow).mutate();
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            popupWindow.setBackgroundDrawable(SpinnerUtils.getDrawable(context, R.drawable.ms__drawable));
        } else {
            popupWindow.setBackgroundDrawable(SpinnerUtils.getDrawable(context, R.drawable.ms__drop_down_shadow));
        }
    ...
    public void setBackgroundColor(int color) {
        ...
        if (background instanceof StateListDrawable) { // pre-L
            try {
                ...
                int[] colors = {SpinnerUtils.darker(color, 0.85f), color};
                for (int i = 0; i < colors.length; i++) {
                    ColorDrawable drawable = (ColorDrawable) getStateDrawable.invoke(background, i);
                    drawable.setColor(colors[i]);
                }
            } catch (Exception e) {
                LogUtils.e("MaterialSpinner", "Error setting background color" + e);
            }
